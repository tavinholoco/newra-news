...
- **Próximos passos:** (1) **`feat/v2-editorial-api`** — `GET /api/home`, `/api/trending` (etapa 1) e `/api/news/:id/related`. **A Fase 3 está bloqueada nela**: abre com `HeroStory`/`DailyBrief`/`TrendingList`, que consomem esses endpoints; (2) **Fase 3 (Home)** depois disso; (3) Item 8 — deploy da newsletter (domínio verificado no Resend)
