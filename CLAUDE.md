...
- **Pendências da Fase 1 fechadas (2026-08-20):** (a) **Lighthouse por rota** medido contra produção pelo mesmo workflow da §3.1 — **acessibilidade sobe em 3 das 5 rotas e não cai em nenhuma**, `/about` fecha em 100, e o `color-contrast` some de `/news` e `/article` (sobra o `heading-order`, que é da Fase 3); performance de pé, com 2 rotas 1 ponto abaixo, dentro do ruído entre execuções. Tabela em `docs/v2/01-design-tokens.md` §12. (b) **Baseline visual da V2** — 39 capturas de produção em `docs/v2/baseline-v2/`, a referência das Fases 2–7
...
- **Testes:** 489 testes em 53 suites (368 API em 32 suites + 121 web em 21 suites — todos passando)
