...
import { adminInvariantsRoutes } from './invariants';
import { adminPipelineRoutes } from './pipeline';
import { adminSourcesRoutes } from './sources';
 ...
 * moravam em `pipeline.ts`. Com a Fase 5 são três grupos de rota (quatro com a
 * Fase 11, cinco com a 6), e repetir as duas linhas em cada um seria voltar ao hábito por
 * rota que o prefixo existe para acabar. Elas registram **uma vez, aqui**, e
 * os filhos herdam o `preHandler` — hook de contexto pai vale para todo
 * `register` abaixo dele.
 *
 ...
 * `adminRoutes` — e alcança `pipeline`, `errors`, `audit`, `sources` e
 * `invariants`, e só eles. A guarda continua a mesma: `authorization-matrix.test.ts` enumera o
...
export async function adminRoutes(app: FastifyInstance) {
  ...
  await app.register(adminSourcesRoutes, { prefix: '/sources' });
  await app.register(adminInvariantsRoutes, { prefix: '/invariants' });
}
